...
trait TimelineTrait
{
     ...
     * @param array<mixed> $options               Query optons from CompanyTimelineEvent
    ...
    private function getTimelineResults(
        ...
        if (isset($options['companyIds'])) {
            $companyColumn = $this->getTableAlias().'.company_id';
            $query->addSelect($companyColumn);
            $query->andWhere(
                $query->expr()->in($companyColumn, $options['companyIds'])
            );
        }
